...
(function () {
    ...
    try {
      ...
      body.innerHTML = `
        <div style="text-align:center; margin-bottom:6px;">
          ...
          ${data.title ? `<div style="font-family:var(--font-mono); font-size:12px; color:var(--accent); margin-top:2px;">${escapeHtml(data.title)}</div>` : ""}
          <div style="font-family:var(--font-mono); font-size:11px; color:var(--muted); margin-top:4px;">${t("memberSince")} ${formatDate(data.createdAt)}</div>
        </div>
  ...
  function renderProfileCorner() {
    ...
    if (username && workerReady()) {
      fetch(`${WORKER_URL}/users?username=${encodeURIComponent(username)}`)
        .then(res => res.ok ? res.json() : null)
        .then(data => {
          if (data && data.title) el.textContent = `👤 ${username} · ${data.title}`;
        })
        .catch(() => {}); // fine if this fails, corner just shows the plain username
    }
  }
