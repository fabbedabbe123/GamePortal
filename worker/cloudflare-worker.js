  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/scores" && request.method === "POST") {
        ...
        // Look up the player's current title (if any) so the leaderboard can
        // show it without needing a separate request per row.
        let title = "";
        try {
          const userRaw = await env.SHELF_KV.get(`user:${nickname.toLowerCase()}`);
          if (userRaw) title = JSON.parse(userRaw).title || "";
        } catch (e) { /* fine, just show no title */ }
        ...
        if (existingIdx === -1) {
          list.push({ nickname, score, ts: Date.now(), title });
        } else if (isBetter(score, list[existingIdx].score)) {
          list[existingIdx] = { nickname, score, ts: Date.now(), title };
        } else {
          // Score didn't improve, but keep the title fresh in case it changed.
      ...
      if (url.pathname === "/users" && request.method === "GET") {
        ...
        const data = JSON.parse(raw);
        return json({ username: data.username, createdAt: data.createdAt, stats: data.stats, achievements: data.achievements, title: data.title || "" });
      }

      // ---------- Accounts: admin sets/clears a user's title ----------
      if (url.pathname === "/users/title" && request.method === "POST") {
        const auth = request.headers.get("Authorization") || "";
        if (auth !== `Bearer ${env.ADMIN_KEY}`) return json({ error: "unauthorized" }, 401);
        const body = await request.json();
      ...
      // ---------- Friends: get my own friends/requests (private, secret required) ----------
      if (url.pathname === "/friends" && request.method === "GET") {
        const username = url.searchParams.get("username") || "";
        const secret = url.searchParams.get("secret") || "";
        if (!username || !secret) return json({ error: "invalid payload" }, 400);
        const raw = await env.SHELF_KV.get(`user:${username.toLowerCase()}`);
        if (!raw) return json({ error: "not found" }, 404);
        const data = JSON.parse(raw);
        if (data.secret !== secret) return json({ error: "unauthorized" }, 401);
      ...
      if (url.pathname === "/friends/request" && request.method === "POST") {
        if (!(await checkRateLimit(env, request, "friendreq", 30, 600))) {
          return json({ error: "too many requests, slow down" }, 429);
        }
        const body = await request.json();
        const username = String(body.username || "");
        ...
        if (!username || !secret || !target) return json({ error: "invalid payload" }, 400);
        if (username.toLowerCase() === target.toLowerCase()) return json({ error: "cannot friend yourself" }, 400);

        const meKey = `user:${username.toLowerCase()}`;
        const meRaw = await env.SHELF_KV.get(meKey);
        if (!meRaw) return json({ error: "not found" }, 404);
        const me = JSON.parse(meRaw);
        if (me.secret !== secret) return json({ error: "unauthorized" }, 401);

        const targetKey = `user:${target.toLowerCase()}`;
        const targetRaw = await env.SHELF_KV.get(targetKey);
        if (!targetRaw) return json({ error: "target not found" }, 404);
        const them = JSON.parse(targetRaw);

        me.friends = me.friends || []; me.incoming = me.incoming || []; me.outgoing = me.outgoing || [];
        them.friends = them.friends || []; them.incoming = them.incoming || []; them.outgoing = them.outgoing || [];

        if (me.friends.includes(them.username)) return json({ error: "already friends" }, 409);
        if (me.outgoing.includes(them.username)) return json({ error: "already requested" }, 409);

        // If they already requested me, this just accepts it instead of double-requesting.
        if (me.incoming.includes(them.username)) {
          me.incoming = me.incoming.filter(u => u !== them.username);
          them.outgoing = them.outgoing.filter(u => u !== me.username);
        ...
        await env.SHELF_KV.put(meKey, JSON.stringify(me));
        await env.SHELF_KV.put(targetKey, JSON.stringify(them));
        return json({ ok: true });
      }

      // ---------- Friends: accept a request ----------
      if (url.pathname === "/friends/accept" && request.method === "POST") {
        const body = await request.json();
        const username = String(body.username || "");
        const secret = String(body.secret || "");
        const from = String(body.from || "").trim();
        if (!username || !secret || !from) return json({ error: "invalid payload" }, 400);
        ...
        const fromKey = `user:${from.toLowerCase()}`;
        const fromRaw = await env.SHELF_KV.get(fromKey);
        if (!fromRaw) return json({ error: "requester not found" }, 404);
        const them = JSON.parse(fromRaw);
        ...
        await env.SHELF_KV.put(meKey, JSON.stringify(me));
        await env.SHELF_KV.put(fromKey, JSON.stringify(them));
        return json({ ok: true });
      }

      // ---------- Friends: decline a request (or remove a friend) ----------
      if (url.pathname === "/friends/decline" && request.method === "POST") {
        const body = await request.json();
        const username = String(body.username || "");
        const secret = String(body.secret || "");
        const other = String(body.from || body.target || "").trim();
        if (!username || !secret || !other) return json({ error: "invalid payload" }, 400);
        ...
        const otherKey = `user:${other.toLowerCase()}`;
        const otherRaw = await env.SHELF_KV.get(otherKey);
        if (otherRaw) {
          const them = JSON.parse(otherRaw);
          them.incoming = (them.incoming || []).filter(u => u !== me.username);
          them.outgoing = (them.outgoing || []).filter(u => u !== me.username);
          them.friends = (them.friends || []).filter(u => u !== me.username);
          await env.SHELF_KV.put(otherKey, JSON.stringify(them));
        }

        me.incoming = (me.incoming || []).filter(u => u.toLowerCase() !== other.toLowerCase());
        me.outgoing = (me.outgoing || []).filter(u => u.toLowerCase() !== other.toLowerCase());
        me.friends = (me.friends || []).filter(u => u.toLowerCase() !== other.toLowerCase());
        await env.SHELF_KV.put(meKey, JSON.stringify(me));
        return json({ ok: true });
      }

      // ---------- Challenges: create one (must already be friends) ----------
      if (url.pathname === "/challenges" && request.method === "POST") {
        if (!(await checkRateLimit(env, request, "challenge", 20, 600))) {
          return json({ error: "too many requests, slow down" }, 429);
        }
        const body = await request.json();
        const username = String(body.username || "");
        ...
        const better = body.better === "lower" ? "lower" : "higher";
        if (!username || !secret || !target || !game || !Number.isFinite(targetScore)) {
          return json({ error: "invalid payload" }, 400);
        }
        ...
        const me = JSON.parse(meRaw);
        if (me.secret !== secret) return json({ error: "unauthorized" }, 401);
        if (!(me.friends || []).some(f => f.toLowerCase() === target.toLowerCase())) {
          return json({ error: "not friends" }, 400);
        }
        ...
        const challenge = {
          id, from: me.username, to: them.username, game, targetScore, better,
          status: "pending", createdAt: Date.now(), result: null
        };

        me.challenges = me.challenges || [];
        them.challenges = them.challenges || [];
        me.challenges.unshift(challenge);
        them.challenges.unshift(challenge);
      ...
      // ---------- Challenges: resolve (the challenged player reports their score) ----------
      if (url.pathname === "/challenges/resolve" && request.method === "POST") {
        const body = await request.json();
        const username = String(body.username || "");
        const secret = String(body.secret || "");
        const id = String(body.id || "");
        const myScore = Number(body.myScore);
        if (!username || !secret || !id || !Number.isFinite(myScore)) {
          return json({ error: "invalid payload" }, 400);
        }
        ...
        const idx = (me.challenges || []).findIndex(c => c.id === id && c.to === me.username && c.status === "pending");
        if (idx === -1) return json({ error: "challenge not found" }, 404);
        const challenge = me.challenges[idx];

        const won = challenge.better === "lower" ? myScore < challenge.targetScore : myScore > challenge.targetScore;
        challenge.status = "completed";
        challenge.result = { opponentScore: myScore, winner: won ? challenge.to : challenge.from };
        me.challenges[idx] = challenge;
        await env.SHELF_KV.put(meKey, JSON.stringify(me));

        // Mirror the same update into the challenger's copy of the record.
        const fromKey = `user:${challenge.from.toLowerCase()}`;
        const fromRaw = await env.SHELF_KV.get(fromKey);
        if (fromRaw) {
          const them = JSON.parse(fromRaw);
          const theirIdx = (them.challenges || []).findIndex(c => c.id === id);
          if (theirIdx !== -1) {
            them.challenges[theirIdx] = challenge;
            await env.SHELF_KV.put(fromKey, JSON.stringify(them));
          }
        }

        return json({ ok: true, result: challenge.result });
      }
