...
(function () {
  ...
  const STRINGS = {
    ...
    titleMissingUsername: { sv: "Skriv in ett användarnamn.", en: "Enter a username." },
    titleUserNotFound: { sv: "Hittar inget konto med det användarnamnet.", en: "No account found with that username." },
    titleSetDone: { sv: "Titel satt!", en: "Title set!" },
    titleClearedDone: { sv: "Titeln borttagen.", en: "Title removed." },
...
`;
  ...
  // ---------- Manage games (edit metadata, enable/disable) ----------
  const mgGameSelect = document.getElementById("mg-game-select");
  const mgTitleInput = document.getElementById("mg-title");
  const mgCategoryInput = document.getElementById("mg-category");
  const mgDescSvInput = document.getElementById("mg-desc-sv");
  const mgDescEnInput = document.getElementById("mg-desc-en");
  const mgColorInput = document.getElementById("mg-color");
  const mgDisabledToggle = document.getElementById("mg-disabled-toggle");
  const mgSaveBtn = document.getElementById("mg-save-btn");
  const mgStatus = document.getElementById("mg-status");

  function populateManageGamesSelect() {
    if (typeof GAMES === "undefined") return;
    mgGameSelect.innerHTML = GAMES.map(g => `<option value="${g.id}">${g.title}${g.disabled ? " (av)" : ""}</option>`).join("");
    if (GAMES.length) loadGameIntoForm(GAMES[0].id);
  }

  function loadGameIntoForm(id) {
    const game = GAMES.find(g => g.id === id);
    if (!game) return;
    mgTitleInput.value = game.title || "";
    mgCategoryInput.value = game.category || "";
    mgDescSvInput.value = game.description || "";
    mgDescEnInput.value = game.descriptionEn || "";
    mgColorInput.value = game.color || "#4fc9c0";
    mgDisabledToggle.checked = !!game.disabled;
    mgStatus.textContent = "";
  }

  mgGameSelect.addEventListener("change", () => loadGameIntoForm(mgGameSelect.value));

  document.querySelector('[data-tab="managegames"]').addEventListener("click", populateManageGamesSelect);

  function serializeGameEntry(g) {
    const keys = Object.keys(g);
    const lines = keys.map((key, i) => {
      const isLast = i === keys.length - 1;
      const valueStr = key === "thumb" ? "`" + g.thumb + "`" : JSON.stringify(g[key]);
      return `    ${key}: ${valueStr}` + (isLast ? "" : ",");
    });
    return "  {\n" + lines.join("\n") + "\n  }";
  }

  function serializeGamesJs(gamesArray) {
    const header = "// ============================================================\n" +
      "// GAME REGISTRY\n" +
      "// Managed via the portal's admin panel (Hantera spel) and by hand.\n" +
      "// ============================================================\n\n" +
      "const GAMES = [\n";
    const body = gamesArray.map(serializeGameEntry).join(",\n");
    return header + body + "\n];\n";
  }

  async function fetchGamesArray() {
    const file = await ghApi("games.js");
    const text = fromBase64Utf8(file.content);
    const match = text.match(/const GAMES = (\[[\s\S]*\]);/);
    if (!match) throw new Error("Could not parse games.js");
    const arr = new Function("return " + match[1])();
    return { arr, sha: file.sha };
  }

  mgSaveBtn.addEventListener("click", async () => {
    const id = mgGameSelect.value;
    if (!id) return;
    mgSaveBtn.disabled = true;
    mgStatus.className = "admin-status pending";
    mgStatus.textContent = "...";
    try {
      const { arr, sha } = await fetchGamesArray();
      const idx = arr.findIndex(g => g.id === id);
      if (idx === -1) throw new Error("Game not found in games.js");

      arr[idx].title = mgTitleInput.value.trim();
      arr[idx].category = mgCategoryInput.value.trim();
      arr[idx].description = mgDescSvInput.value.trim();
      arr[idx].descriptionEn = mgDescEnInput.value.trim();
      arr[idx].color = mgColorInput.value;
      arr[idx].disabled = mgDisabledToggle.checked;

      const newText = serializeGamesJs(arr);
      await ghApi("games.js", {
        method: "PUT",
        body: JSON.stringify({
          message: `Update game: ${id}`,
          content: toBase64Utf8(newText),
          sha
        })
      });

      if (typeof GAMES !== "undefined") {
        const liveIdx = GAMES.findIndex(g => g.id === id);
        if (liveIdx !== -1) Object.assign(GAMES[liveIdx], arr[idx]);
        if (window.ShelfPortal && window.ShelfPortal.refresh) window.ShelfPortal.refresh();
      }
      populateManageGamesSelect();

      mgStatus.className = "admin-status success";
      mgStatus.textContent = "Sparat!";
    } catch (err) {
      mgStatus.className = "admin-status error";
      mgStatus.textContent = t("genericError") + err.message;
    } finally {
      mgSaveBtn.disabled = false;
    }
  });

  // ---------- Titles (admin grants a badge/title next to a username) ----------
  const titleUsernameInput = document.getElementById("title-username");
  ...
  titleSaveBtn.addEventListener("click", async () => {
    const adminKey = getAdminKey();
    const username = titleUsernameInput.value.trim();
    const title = titleTextInput.value.trim();

    if (typeof WORKER_URL === "undefined" || WORKER_URL.includes("REPLACE-ME")) {
      titleStatus.className = "admin-status error";
      titleStatus.textContent = t("feedbackNotConfigured");
      return;
    }
    if (!adminKey) {
      titleStatus.className = "admin-status error";
      titleStatus.textContent = t("feedbackNoAdminKey");
      return;
    }
    if (!username) {
      titleStatus.className = "admin-status error";
      titleStatus.textContent = t("titleMissingUsername");
      return;
    }

    titleSaveBtn.disabled = true;
    titleStatus.className = "admin-status pending";
    titleStatus.textContent = "...";
    try {
      const res = await fetch(`${WORKER_URL}/users/title`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${adminKey}` },
        body: JSON.stringify({ username, title })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error === "not found" ? t("titleUserNotFound") : `Worker: ${res.status}`);
      titleStatus.className = "admin-status success";
      titleStatus.textContent = title ? t("titleSetDone") : t("titleClearedDone");
    } catch (err) {
      titleStatus.className = "admin-status error";
      titleStatus.textContent = t("genericError") + err.message;
    } finally {
