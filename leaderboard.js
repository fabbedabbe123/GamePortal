    ...
    try {
      ...
      list.innerHTML = scores.map((entry, i) => `
        <div class="leaderboard-row ${entry.nickname === myName ? "me" : ""}">
          ...
          <span class="lb-name" style="cursor:pointer;" data-open-profile="${escapeHtml(entry.nickname)}">
            ${escapeHtml(entry.nickname)}${entry.title ? `<br><span style="font-size:10px; color:var(--accent); font-weight:400;">${escapeHtml(entry.title)}</span>` : ""}
          </span>
